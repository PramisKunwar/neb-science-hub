...
const popularSearchTerms = [
  ...
  "Mechanics", "Thermodynamics", "Optics", "Calculus",
  "Organic Chemistry", "Algebra", "Waves"
];
...
export default function Search() {
  ...
  const updateSuggestions = useCallback((searchQuery: string, searches: string[]) => {
    if (searchQuery.trim().length > 1) {
      ...
      const matchedTerms = popularSearchTerms.filter(term => 
        term.toLowerCase().includes(searchQuery.toLowerCase()) && 
        term.toLowerCase() !== searchQuery.toLowerCase()
      );
      
      // Find matching recent searches
      const matchedRecent = searches.filter(term => 
        term.toLowerCase().includes(searchQuery.toLowerCase()) && 
        term.toLowerCase() !== searchQuery.toLowerCase() &&
        !matchedTerms.some(popularTerm => popularTerm.toLowerCase() === term.toLowerCase())
      );
  ...
  const displayedPopularTerms = useMemo(() => 
    popularSearchTerms.slice(0, 6), 
    []
  );
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 py-8 relative" ref={searchRef}>
      ...
      <form onSubmit={handleSearch} className="flex space-x-2">
        ...
        <Input
          ...
          onChange={(e) => setQuery(e.target.value)}
              ...
              className="pl-10 pr-10 bg-white border-2 border-gray-300 focus:border-nebBlue focus:ring-2 focus:ring-blue-100 h-12 text-base group-hover:border-gray-400 transition-all duration-200"
          ...
          <Button 
            type="submit" 
            className="bg-nebBlue text-white hover:bg-blue-800 transition-all duration-200 h-12 px-6 shadow-sm hover:shadow transform hover:-translate-y-1 hover:scale-105 font-medium"
          >
      ...
      {isInputFocused && !query && !showResults && (
        <Card className="mt-2 absolute w-full z-10 border-2 border-gray-200 shadow-md animate-fadeIn">
              ...
              {recentSearches.length > 0 ? (
                <ul className="space-y-1">
                  {recentSearches.map((term, index) => (
                    <li key={index}>
                      <button
                        onClick={(e) => handleRecentSearchClick(term, e)}
                        className="flex items-center w-full p-2 hover:bg-blue-50 rounded-md text-left group transition-all duration-200 hover:pl-3"
                      >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {displayedPopularTerms.map((term, index) => (
                  <Badge 
                    key={index}
                    className="bg-gray-100 hover:bg-blue-100 text-gray-800 hover:text-nebBlue border border-gray-200 cursor-pointer transition-all duration-200 hover:scale-110 hover:shadow-sm"
                    onClick={(e) => handleSuggestionClick(term, e as React.MouseEvent)}
                  >
                    {term}
                  </Badge>
                ))}
      ...
      {isInputFocused && query && suggestions.length > 0 && !showResults && (
        <Card className="mt-2 absolute w-full z-10 border-2 border-gray-200 shadow-md animate-fadeIn">
              ...
              {suggestions.map((suggestion, index) => (
                <li key={index}>
                  <button
                    ...
                    className={`flex items-center w-full p-3 hover:bg-blue-50 border-b border-gray-100 last:border-b-0 text-left transition-all duration-200 group ${hoveredSuggestion === index ? 'bg-blue-50' : ''}`}
                  >
      ...
      {/* Search Results */}
      {showResults && results && (
        <Card className="mt-2 absolute w-full z-10 max-h-[60vh] overflow-y-auto border-2 border-gray-200 shadow-md animate-fadeIn">
          <CardContent className="p-4">
                      ...
                      {results.subjects.map((subject) => (
                        <li key={subject.id}>
                          <Link 
                            ...
                            className="flex items-center p-2 hover:bg-blue-50 rounded-md transition-all duration-200 group hover:pl-3"
